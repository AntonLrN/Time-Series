...
% d = 2 or, maybe r = 0 , unsure and s = 0
...
[ foundModel, ey, ~, pacfEst ] = estimateBJ( modely, modelx', [1], [1], [0 0 1], [1], 'BJ model 1', noLags );
var_ey = var(ey);
acf_pacf_norm(ey);
% %% Let us try to add a1, seems to be a strong pacf there
% [ foundModel, ey, ~, pacfEst ] = estimateBJ( modely, modelx', [1 0 0 0 0 0 1], [1], [0 0 1 1], [1 1 ], 'BJ model 1', noLags );
% var_ey = var(ey);
% acf_pacf_norm(ey);
...
%%
y_true = filter( [1], [1 -1], yhat)
y_true2 = filter( [1], [1 -1], valy)
plot(y_true)
hold on
plot(y_true2)
hold off